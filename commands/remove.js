...
module.exports = {
    data: new SlashCommandBuilder().setName("remove").setDescription("Removes song at given position").addIntegerOption(option =>
    ...
    async execute(interaction) {
        ...
        if (songPosition > 0 && songPosition <= QueueManager.getInstance().getSongCountInQueue(interaction.guildId)) {
            QueueManager.getInstance().removeSong(interaction.guildId, interaction.channel, voiceChannel, songPosition);
            return await interaction.followUp({content: "Removed song"});
        } else {
